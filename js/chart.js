...
const ChartModule = (function () {
  ...
  let _layoutTmr  = null;   // polling timer handle
...
._cbk{padding:6px 16px;border-radius:999px;border:1.5px solid rgba(255,140,66,.35);background:rgba(255,140,66,.12);color:var(--hc-ac,#ff8c42);font-size:13px;font-weight:800;font-family:'Cairo',sans-serif;cursor:pointer;-webkit-tap-highlight-color:transparent;transition:background .15s;}
._cbk:hover{background:rgba(255,140,66,.22);}
._cbk:active{opacity:.7;}
._clk{background:var(--bg-elev,#161616);border:1.5px solid var(--border-strong,#3d3d3d);border-radius:10px;color:var(--text-secondary,#a0a0a0);font-size:16px;padding:5px 9px;line-height:1;cursor:pointer;-webkit-tap-highlight-color:transparent;}
...
._ctb{display:flex;align-items:center;gap:8px;padding:7px 12px;background:var(--bg-card,#0d0d0d);border-bottom:1px solid var(--border,#2a2a2a);flex-shrink:0;direction:rtl;}
...
._cqi{width:80px;font-family:'IBM Plex Mono',monospace;font-size:19px;font-weight:800;text-align:center;direction:ltr;background:var(--bg-input,#1e1e1e);border:2px solid var(--border-strong,#3d3d3d);border-radius:10px;padding:5px 4px;color:var(--text-primary,#f5f5f5);outline:none;-webkit-tap-highlight-color:transparent;}
...
._ctw{flex:1;min-height:0;position:relative;overflow:hidden;background:var(--bg-app,#000);direction:ltr;}
#_tvC{position:absolute;top:0;left:0;direction:ltr;overflow:hidden;}
#_tvC iframe{border:none!important;display:block;}
._cfo{position:absolute;inset:0;z-index:90;display:flex;align-items:flex-end;justify-content:center;background:rgba(0,0,0,.72);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);direction:rtl;}
._cfc{background:var(--bg-card,#0d0d0d);border-top:2px solid var(--border-strong,#3d3d3d);border-radius:22px 22px 0 0;width:100%;max-width:520px;padding:14px 16px max(28px,env(safe-area-inset-bottom));animation:_cfSU .22s cubic-bezier(.4,0,.2,1);}
@keyframes _cfSU{from{transform:translateY(100%)}to{transform:none}}
._cfh{width:32px;height:4px;background:var(--border-strong,#3d3d3d);border-radius:999px;margin:0 auto 12px;}
._cft{font-size:17px;font-weight:900;margin-bottom:12px;color:var(--text-primary,#f5f5f5);}
._cfr{background:var(--bg-input,#1e1e1e);border-radius:12px;padding:8px 14px;margin-bottom:14px;border:1px solid var(--border,#2a2a2a);}
...
._cfv{font-family:'IBM Plex Mono',monospace;font-size:14px;font-weight:900;color:var(--text-primary,#f5f5f5);}
._cfv.g{color:#00e676;}._cfv.r{color:#ff3d3d;}._cfv.w{color:#ffd600;}
._cfb{display:grid;grid-template-columns:1fr 1fr;gap:10px;}
._cfca{padding:14px;border-radius:999px;border:1.5px solid var(--border-strong,#3d3d3d);background:var(--bg-elev,#161616);color:var(--text-secondary,#a0a0a0);font-size:14px;font-weight:700;cursor:pointer;font-family:'Cairo',sans-serif;transition:all .15s;}
._cfex{padding:14px;border-radius:999px;border:none;color:#fff;font-size:14px;font-weight:900;cursor:pointer;font-family:'Cairo',sans-serif;display:flex;align-items:center;justify-content:center;gap:5px;transition:filter .15s;}
._cfex.g{background:linear-gradient(135deg,#00e676,#007c3a);box-shadow:0 2px 12px rgba(0,200,80,.3);}
._cfex.r{background:linear-gradient(135deg,#ff3d3d,#a00000);box-shadow:0 2px 12px rgba(220,50,50,.3);}
...
<div class="_ctw" id="_cWrap">
  ...
  function _btnPx() {
    ...
    if (!p) return;
    if (b) b.textContent = '$' + p.toFixed(a.pxDp);
    if (s) s.textContent = '$' + p.toFixed(a.pxDp);
  }
  ...
  function _rtConn(sym, res) {
    ...
    try {
      ...
      _rtWs.onerror = () => {};
    } catch (e) { console.warn('[ChartRT]', e.message); }
  }
  ...
  /* ── ResizeObserver: keep widget dimensions in sync with wrapper ── */
  function _setupResize() {
    ...
    _ro = new ResizeObserver(entries => {
      ...
      if (cont) {
        cont.style.width  = Math.floor(width)  + 'px';
        cont.style.height = Math.floor(height) + 'px';
      }

      /* Resize the TradingView widget */
  ...
  /* ── Poll until container has real pixel dimensions ── */
  function _waitLayout(callback, attempts) {
    ...
    if (wrap && _visible) {
      ...
      if (r.width > 10 && r.height > 10) {
        callback(Math.floor(r.width), Math.floor(r.height));
        return;
      }
    }

    if (!_visible) return; /* chart was closed while waiting */

    if (attempts < 100) {
      /* retry every ~16ms (one frame), up to ~1.6 seconds */
      _layoutTmr = setTimeout(() => _waitLayout(callback, attempts + 1), 16);
    } else {
      /* hard fallback: derive from window — always non-zero */
      const sc = document.getElementById('chartScreen');
      const w  = sc ? sc.clientWidth  : window.innerWidth;
      const h  = sc ? sc.clientHeight : window.innerHeight;
      /* subtract nav (~48px) + trade bar (~68px) */
      callback(w, Math.max(200, h - 120));
    }
  }
  ...
  function _initWidget(sym) {
    ...
    if (typeof TradingView === 'undefined' || typeof TradingView.widget !== 'function') {
      ...
      if (w) w.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;
                    height:100%;color:#ff8c42;font-size:14px;font-weight:700;
                    font-family:Cairo,sans-serif;text-align:center;padding:20px;gap:12px;direction:rtl;">
          <div style="font-size:32px;">⚠️</div>
          <div>تعذّر تحميل مكتبة الرسم البياني<br>
            <small style="opacity:.6;font-size:11px;line-height:1.8;">
              تأكد من وجود<br>/charting_library/charting_library.standalone.js
            </small>
          </div>
        </div>`;
      return;
    ...
    /* ✅ FIX: wait until wrapper has real pixel dimensions before creating widget */
  ...
  function _doInit(sym, w, h) {
    ...
    /* ✅ FIX: set EXPLICIT pixel dimensions — no reliance on CSS flex/% */
    ...
    try {
      /* ✅ FIX: explicit width + height instead of autosize:true
         autosize reads container at init time — if it's 0, chart never renders.
         We provide exact px dimensions and handle resize via ResizeObserver. */
      _widget = new TradingView.widget({
        ...
        overrides: {
          ...
          'scalesProperties.textColor':    dark ? '#a0a0a0' : '#333333',
      ...
      _widget.onChartReady(() => {
        /* attach ResizeObserver now that widget is fully ready */
        _setupResize();
        _btnPx();
        try {
