...
/*
  chart.js — TradingView Advanced Charts
  - Hyperliquid custom datafeed
  - No lightweight-charts
  - Proper async init, no race conditions
  - Container 100% width/height
  - Dark/Light adaptive
  - Default 1H, no indicators
...
const ChartModule = (function () {
  ...
  function _coin(s)   { return (typeof ASSETS!=='undefined'&&ASSETS[s]?.coin)||('xyz:'+s); }
  function _gram(s)   { return !!(typeof ASSETS!=='undefined'&&ASSETS[s]?.gram); }
  function _asset(s)  { return (typeof ASSETS!=='undefined'&&ASSETS[s])||{pxDp:2,szDp:2,name:s,unit:'',icon:'📊',lev:10,presets:[1],idx:0,cross:true}; }
  function _price(s)  { return (typeof State!=='undefined')?(State.prices[s]?.mid||0):0; }
...
._cn{display:flex;align-items:center;justify-content:space-between;padding:7px 12px;background:var(--bg-card,#0d0d0d);border-bottom:1px solid var(--border,#2a2a2a);flex-shrink:0;gap:8px;direction:rtl;min-height:46px;}
._cnl,._cnr{display:flex;align-items:center;gap:8px;}
._cbk{padding:5px 13px;border-radius:999px;border:1.5px solid rgba(255,140,66,.3);background:rgba(255,140,66,.1);color:var(--hc-ac,#ff8c42);font-size:13px;font-weight:800;font-family:'Cairo',sans-serif;cursor:pointer;-webkit-tap-highlight-color:transparent;}
._cbk:active{opacity:.7;transform:scale(.95);}
._clk{background:var(--bg-elev,#161616);border:1.5px solid var(--border-strong,#3d3d3d);border-radius:var(--r-sm,12px);color:var(--text-secondary,#a0a0a0);font-size:16px;padding:4px 8px;line-height:1;cursor:pointer;-webkit-tap-highlight-color:transparent;}
._clk:active{opacity:.6;}
._cai{display:flex;align-items:center;gap:6px;}
._cic{font-size:17px;line-height:1;}
._cnm{font-size:13px;font-weight:900;color:var(--text-primary,#f5f5f5);}
...
._cb{flex:1;min-height:50px;padding:6px 8px;border-radius:13px;border:none;font-family:'Cairo',sans-serif;cursor:pointer;color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;-webkit-tap-highlight-color:transparent;transition:transform .11s,filter .11s;}
._cb:active{transform:scale(.93);filter:brightness(.87);}
._buy{background:linear-gradient(150deg,#00e676,#007c3a);box-shadow:0 2px 10px rgba(0,200,80,.28);}
._sel{background:linear-gradient(150deg,#ff3d3d,#a00000);box-shadow:0 2px 10px rgba(220,50,50,.28);}
._cdir{font-size:14px;font-weight:900;line-height:1;}
._cpx{font-family:'IBM Plex Mono',monospace;font-size:10px;opacity:.82;font-weight:700;}
._cmd{flex:1.1;display:flex;flex-direction:column;align-items:center;gap:3px;}
._cql{font-size:9px;color:var(--text-muted,#555);font-weight:700;letter-spacing:.5px;text-transform:uppercase;}
._cqr{display:flex;align-items:center;gap:4px;}
._cqi{width:78px;font-family:'IBM Plex Mono',monospace;font-size:18px;font-weight:800;text-align:center;direction:ltr;background:var(--bg-input,#1e1e1e);border:2px solid var(--border-strong,#3d3d3d);border-radius:10px;padding:5px 4px;color:var(--text-primary,#f5f5f5);outline:none;-webkit-tap-highlight-color:transparent;}
._cqi:focus{border-color:var(--hc-ac,#ff8c42);box-shadow:0 0 0 2px rgba(255,140,66,.18);}
._cqu{font-size:10px;color:var(--text-secondary,#a0a0a0);font-weight:800;white-space:nowrap;}
._ctw{flex:1;min-height:0;position:relative;overflow:hidden;}
#_tvC{width:100%;height:100%;position:absolute;inset:0;}
#_tvC iframe{border:none!important;width:100%!important;height:100%!important;}
._cfo{position:absolute;inset:0;z-index:90;display:flex;align-items:flex-end;justify-content:center;background:rgba(0,0,0,.68);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);direction:rtl;}
._cfc{background:var(--bg-card,#0d0d0d);border-top:2px solid var(--border-strong,#3d3d3d);border-radius:22px 22px 0 0;width:100%;max-width:480px;padding:13px 15px 28px;animation:_cfSU .22s cubic-bezier(.4,0,.2,1);}
...
._cft{font-size:16px;font-weight:900;margin-bottom:9px;color:var(--text-primary,#f5f5f5);}
._cfr{background:var(--bg-input,#1e1e1e);border-radius:12px;padding:7px 11px;margin-bottom:13px;display:flex;flex-direction:column;}
...
._cfb{display:grid;grid-template-columns:1fr 1fr;gap:9px;}
._cfca{padding:12px;border-radius:999px;border:1.5px solid var(--border-strong,#3d3d3d);background:var(--bg-elev,#161616);color:var(--text-secondary,#a0a0a0);font-size:13px;font-weight:700;cursor:pointer;font-family:'Cairo',sans-serif;}
._cfex{padding:12px;border-radius:999px;border:none;color:#fff;font-size:13px;font-weight:900;cursor:pointer;font-family:'Cairo',sans-serif;display:flex;align-items:center;justify-content:center;gap:5px;}
...
._cfspin{width:13px;height:13px;border:2px solid rgba(255,255,255,.3);border-top-color:#fff;border-radius:50%;animation:_spinC .7s linear infinite;}
@keyframes _spinC{to{transform:rotate(360deg)}}
`;
  ...
  function _build() {
    ...
    document.getElementById('_cLck').onclick  = () => typeof lockApp==='function' && lockApp();
    document.getElementById('_cBuy').onclick  = () => _cf(true);
    document.getElementById('_cSel').onclick  = () => _cf(false);
  }

  function _header(sym) {
    const a = _asset(sym);
    const i = document.getElementById('_cIco'), n = document.getElementById('_cNam');
    const u = document.getElementById('_cQun'), q = document.getElementById('_cQty');
    if (i) i.textContent = a.icon||'📊';
    if (n) n.textContent = a.name||sym;
    if (u) u.textContent = a.unit||'';
    if (q) q.value = a.presets?.[0]??1;
    _pxBtn();
  }

  function _pxBtn() {
    const p = _price(_sym), a = _asset(_sym);
    ...
    try {
      ...
      _rtWs.onclose=()=>{
        if(_visible&&Object.keys(_subs).length)
          _rtTimer=setTimeout(()=>_rtConn(_sym,_currentRes),4000);
      };
      _rtWs.onerror=()=>{};
  ...
  function _initWidget(sym) {
    ...
    /* Critical guard: library must be present */
    if(typeof TradingView==='undefined'||typeof TradingView.widget!=='function') {
      console.error('[Chart] TradingView library not loaded. Ensure /charting_library/charting_library.standalone.js is included before chart.js');
      const w=document.getElementById('_cWrap');
      if(w) w.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#ff8c42;font-family:Cairo,sans-serif;font-size:14px;font-weight:700;text-align:center;padding:20px;">⚠️ مكتبة TradingView غير محملة<br><small style="opacity:.6;font-size:11px;margin-top:6px;display:block;">تحقق من /charting_library/charting_library.standalone.js</small></div>';
      return;
    }

    const cont=document.getElementById('_tvC');
    if(!cont){console.error('[Chart] #_tvC not found');return;}
    ...
    } catch(e){ console.error('[Chart] widget error:',e.message,e); }
  }
  ...
  function _cf(buy) {
    ...
    const a=_asset(_sym), gram=_gram(_sym);
    const mid=_price(_sym);
    ...
    const liqOz=buy?midOz*(1-1/a.lev):midOz*(1+1/a.lev);
    ...
    ov.innerHTML=`<div class="_cfc">
      ...
      <div class="_cfr">
        ...
        <div class="_cfrow"><span class="_cfk">التصفية</span><span class="_cfv ${buy?'r':'g'}">≈ $${liqD}</span></div>
      </div>
      <div class="_cfb">
  ...
  async function _exec(buy,qty) {
    ...
    const mid=_price(_sym), midOz=gram?mid*TL:mid;
  ...
  function switchInterval(iv) {
    if(!_widget) return;
    try{ _widget.onChartReady(()=>{ _widget.activeChart().setResolution(iv,()=>{_currentRes=iv;}); }); }catch{}
  }

  function switchAssetChart(sym) {
    if(!_visible||sym===_sym) return;
    _sym=sym; _header(sym); _initWidget(sym);
  }

  function refreshLines() { /* stub for future position lines */ }
