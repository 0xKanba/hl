/* ═══════════════════════════════════════════════════════════════
   HL Trade · chart.js v7.1
   ✅ زر ↺ لإعادة الضبط — بدون Bug تكبير مفاجئ
   ✅ Crosshair: سعر + تاريخ + وقت كامل UTC+3
   ✅ اليوم يبدأ منتصف الليل UTC+3
   ✅ نصوص أوضح وأسمك
   ✅ حرية تمرير وتكبير كاملة
...
.iv-btn {
  flex:1; padding:5px 3px; border-radius:999px;
...
.cbt-btn {
  flex:1; min-height:54px; padding:7px 5px;
...
.cbt-buy  { background:linear-gradient(150deg,#2da44e,#1a7f37); box-shadow:0 4px 12px rgba(45,164,78,.3); }
.cbt-sell { background:linear-gradient(150deg,#e5534b,#a0281e); box-shadow:0 4px 12px rgba(229,83,75,.3); }
.cbt-dir { font-size:16px; line-height:1; }
...
.cbt-qty-in {
  ...
  border:2px solid var(--ac-dim); border-radius:10px;
  padding:6px 4px; color:var(--text-primary); outline:none;
...
.c-legend {
  ...
  flex-shrink:0; min-height:24px; align-items:center;
  overflow:hidden;
}
...
.cf-rows  {
  ...
  padding:8px 10px;display:flex;flex-direction:column;gap:4px;margin-bottom:10px;
}
...
`;
  ...
  function _toUTC3(tick) {
    return new Date((tick + UTC3) * 1000);
  }

  /* وقت فقط للمحور: 03:45 PM */
  function fmtTimeAxis(tick) {
    const d  = _toUTC3(tick);
  ...
  /* تاريخ فقط للمحور: Jan 12 */
  function fmtDateAxis(tick) {
    const d = _toUTC3(tick);
    return `${_MONTHS[d.getUTCMonth()]} ${d.getUTCDate()}`;
  }

  /* تاريخ + وقت كامل للـ Crosshair: Thu 12 Jan 2025 · 03:45 PM */
  function fmtCrosshair(tick) {
    const d  = _toUTC3(tick);
    const hh = d.getUTCHours(), mm = d.getUTCMinutes();
    const ap = hh >= 12 ? 'PM' : 'AM';
    const h  = hh % 12 || 12;
    return `${_DAYS[d.getUTCDay()]} ${d.getUTCDate()} ${_MONTHS[d.getUTCMonth()]} ${d.getUTCFullYear()} · ${String(h).padStart(2,'0')}:${String(mm).padStart(2,'0')} ${ap} +3`;
  }
  ...
  function startClock() {
    ...
    const tick = () => {
      ...
      const cd   = document.getElementById('_cCountdown');
  ...
  /* ════════════
     ↺ إعادة الضبط — بدون bug
     الطريقة الصحيحة: scrollToRealTime فقط
     بدون إعادة بناء الـ chart
  ════════════ */
  function resetView() {
    if (!_chart) return;
    /* scrollToRealTime تحرك الوقت للنهاية فقط — بدون تكبير مفاجئ */
    _chart.timeScale().scrollToRealTime();
    /* إعادة auto-scale للسعر بشكل ناعم */
    _series?.applyOptions({ autoscaleInfoProvider: () => null });
    if (typeof toast !== 'undefined') toast('↺ عرض آخر الشموع', 'info', 1000);
  }
  ...
  /* ════════════
     إحصائيات اليوم — يبدأ منتصف الليل UTC+3
  ════════════ */
  function getSessionStartSec() {
    /* منتصف الليل UTC+3 = 21:00 UTC من اليوم السابق */
    const nowUTC3  = Date.now() + UTC3 * 1000;
    const d        = new Date(nowUTC3);
    const midnight = Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()); // 00:00 UTC+3
    return Math.floor(midnight / 1000) - UTC3; // تحويل لـ UTC بالثواني
  }
  ...
  function drawDayStats() {
    ...
    if (el) {
      el.innerHTML =
        ...
        `<span style="color:${up?'#26a69a':'#ef5350'};font-size:13px;font-weight:900;">${up?'+':''}${pct}%</span>`;
    }
  }
  ...
  function buildChart(container) {
    ...
    const TXT  = dark ? '#d1d4dc' : '#131722';  // نص أوضح
    ...
    const LBGC = dark ? '#1565c0' : '#1e88e5';  // خلفية تسمية زرقاء واضحة
    ...
    _chart = LightweightCharts.createChart(container, {
      ...
      timeScale: {
        ...
        fixLeftEdge:      true,
        borderVisible:    true,
        /* ✅ محور الأسفل UTC+3 مع AM/PM */
        tickMarkFormatter: (tick, type) => {
          if (type >= 3) return fmtTimeAxis(tick);
          if (type === 2) return fmtDateAxis(tick);
          if (type === 1) return _MONTHS[_toUTC3(tick).getUTCMonth()];
          return _toUTC3(tick).getUTCFullYear().toString();
        },
      },
      handleScroll: {
        mouseWheel:       true,
      ...
      localization: {
        ...
        /* ✅ تسمية المحور السفلي عند الـ Crosshair = تاريخ + وقت كامل */
        timeFormatter: tick => fmtCrosshair(tick),
      },
    ...
    _chart.subscribeCrosshairMove(param => {
      ...
      /* ✅ تاريخ + وقت كامل UTC+3 في الـ Legend */
      el.innerHTML =
        `<span style="color:var(--text-secondary);font-size:10px;font-weight:700;white-space:nowrap;">${fmtCrosshair(param.time)}</span>` +
    ...
    screen.innerHTML = `
      <div class="c-nav">
        <div class="c-nav-row">
          ...
          <div class="c-nav-group">
            <button class="c-reset-btn" id="_cReset" title="إعادة الضبط">↺</button>
        ...
        <div class="c-nav-row">
          ...
          <div style="display:flex;align-items:center;gap:8px;padding-right:4px;">
            <span id="_cCountdown" style="color:var(--ac);font-weight:800;font-size:11px;font-family:monospace;min-width:44px;">—</span>
            <span id="_cClockT"   style="font-family:monospace;font-size:11px;color:var(--text-secondary);">—</span>
          </div>
        </div>
      </div>
