/* ═══════════════════════════════════════
   ...
   ✅ FIX أداء (منصة "فتح صفقة جديدة") — عدّاد عمر السعر (#priceTimer)
      كان يُلغى ويُعاد إنشاؤه عند كل تغيّر سعر فعلي، ويدق كل ثانية.
      بعد حذف بطاقة السعر من الرئيسية لم يعد لعنصره وجود بالـDOM
      إطلاقاً — فبقي مؤقّتاً أبدياً يكتب لعنصر غير موجود (setTxt
      تتجاهله بصمت)، مع دورة clearInterval/setInterval كاملة مع كل
      نبضة سعر بسوق سريع الحركة. الآن لا يُنشأ أصلاً ما لم يوجد
      العنصر فعلاً — الكود يبقى جاهزاً كما هو لو أُعيد العنصر لأي
      شاشة مستقبلاً، بلا أي تكلفة أثناء غيابه.
═══════════════════════════════════════ */
'use strict';
...
function updatePriceUI() {
  ...
  /* ✅ إصلاح — عدّاد الثواني كان يُلغى ويُعاد إنشاؤه عند كل نبضة BBO
     ...
     ✅ FIX أداء إضافي — ولا يُنشأ إطلاقاً ما لم يكن عنصره موجوداً
     بالـDOM أصلاً (راجع تعليق رأس الملف: العنصر حُذف مع بطاقة السعر
     القديمة من الرئيسية، فكان المؤقّت يدق للأبد بلا أي مستهلك). */
  if ($('priceTimer') && _lastTimerMid !== p.mid) {
    _lastTimerMid = p.mid;
    let s = 1;
