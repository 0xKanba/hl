/* ═══════════════════════════════════════
   ...
   ⚠️ CRITICAL FIX — doDeposit كان يستدعي approve()+bridge.deposit() على
      ...
      بالأسفل.
   ✅ FIX (2026-08) — أعيد التحقق من كل مسار إيداع/سحب مباشرة مقابل
      /for-developers/api/bridge2 و/exchange-endpoint الرسميين: عنوان
      الجسر، عنوان USDC، آلية transfer المباشرة، الحد الأدنى $5،
      رسوم السحب $1، ونطاق/ترتيب حقول توقيع withdraw3 (EIP-712) —
      كلها مطابقة 100% بلا أي تعديل مطلوب على منطق التوقيع نفسه.
   ✅ FIX (2026-08) — كان فحص رصيد USDC داخل doDeposit يعمل
      `throw new Error('رصيد USDC غير كافٍ على Arbitrum')` (رسالة عربية
      جاهزة ومحدَّدة)، لكن catch أدناه يمرّرها حتماً عبر _depositErr()
      التي تُطابق فقط كلمات إنجليزية مفتاحية من رسائل ethers/الشبكة
      الخام (insufficient/gas/...). أي نص عربي لا يطابق أي شرط فيها
      فيسقط للرسالة الافتراضية العامة "فشل الإيداع — حاول مجدداً"،
      فتضيع الرسالة المحدَّدة الأصلية. الحالة هذي بالذات (رصيد USDC
      غير كافٍ) هي بالضبط أول ما يواجهه أي مستخدم بريد جديد (محفظة
      Privy فارغة تماماً أول مرة) — الآن toast+return مباشر يتخطى
      catch/_depositErr كلياً لهذي الحالة المعروفة تحديداً، بنفس نمط
      الفحوصات المبكرة الموجود أصلاً بـdoWithdraw. finally يبقى يعمل
      كالمعتاد (تنظيف الزر/اللودر) لأن return داخل try لا يُلغي finally.
   ✅ FIX (2026-08) — أُضيف تنبيه صريح بحاجة القليل من ETH لرسوم غاز
...
/* ════ Deposit ════
   ...
   ✅ FIX (2026-08): فحص الرصيد الآن يُنهي العملية مباشرة (toast+return)
      بدل throw+catch — راجع تعليق رأس الملف لسبب هذا التغيير بالتحديد.
════════════════════════════════════════════════════════════════ */
async function doDeposit() {
  ...
  try {
    ...
    if (bal < raw) {
      /* ✅ رسالة عربية محدَّدة جاهزة — لا تمر عبر _depositErr() (تلك
         مصمَّمة لمطابقة كلمات إنجليزية من رسائل ethers/الشبكة الخام؛
         أي نص عربي يفشل كل مطابقاتها ويسقط للرسالة العامة، فيضيع
         التحديد المفيد هنا تحديداً). return يبقي finally يعمل كالمعتاد. */
    ...
    /* ✅ FIX (2026-08) — تذكير تصدير المفتاح لحساب بريد (Privy) جديد
       كلياً انتقل لهنا (أول إيداع ناجح فعلياً) بدل وقت الاتصال الخام —
       راجع تعليق رأس auth.js لسبب هذا التأجيل. لا تأثير على محفظة
       خارجية (الدالة نفسها تتجاهلها فوراً) ولا على حساب كان مموَّلاً
       أصلاً عند الاتصال (تلقّى تذكيره فوراً هناك، فـflag محلي يمنع
       التكرار هنا). */
